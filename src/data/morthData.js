...
import clusterStateTopData from './cluster-state-top.json';
import clusterDistrictTopData from './cluster-district-top.json';
...
import mortStateIndiaSummaryData from './data/mort-state-india-summary.json';
import mortDistrictIndiaSummaryData from './data/mort-district-india-summary.json';
import clusterDistrictViolationsData from './data/cluster-district-violations.json';
import clusterDistrictFgData from './data/cluster-district-fg.json';

export const MORTH_DATA = {
  summary: mortNationalData.summary,
  india_summary: mortNationalData.india_summary,
  ...
  top20Corridors:
    mortNationalData.top_corridors ||
    mortNationalData.top_20_corridors ||
    mortNationalData.top20Corridors ||
    []
};
...
const scaleSchoolZoneMetrics = (metrics, factor) => {
  ...
  return {
    ...
    accidents_age_6_18: metrics.accidents_age_6_18 || 0,
...
const scaleState = (state, factor) => ({
  ...state,
  india_summary: state.india_summary || {},
...
const buildCategoryDataset = (baseDataset, factor) => ({
  summary: {
    ...
    total_fatal_accidents: toIndianString(toNumber(baseDataset.summary.total_fatal_accidents) * factor),
    total_grievous_accidents: toIndianString(toNumber(baseDataset.summary.total_grievous_accidents) * factor),
  ...
  bars: (baseDataset.bars || []).map((bar) => ({
    ...
    rows_expanded: (bar.rows_expanded || []).map((row) => ({
      ...row,
      value: Math.max(1, Math.round((row.value || 0) * factor))
    }))
  })),
...
export const SCHOOLS_ZONE_DATA = {
  ...
  school_zone_metrics: schoolsZoneNationalData.school_zone_metrics || null,
...
export const CLUSTER_DATA = {
  ...
  top20Clusters: clusterNationalData.top_clusters || clusterNationalData.top20Clusters || []
};
...
const buildComparisonCardData = (seriesRows, category = DEFAULT_CATEGORY) => {
  const [a, b, c] = seriesRows.map((row) => row.state);
  const metric = (state, key) => Number(state?.[key]) || 0;
  const fatalityPercent = (state) => {
    const fatalities = Number(state?.total_fatalities) || 0;
    const accidents = Number(state?.total_accidents) || Number(state?.fg_covered) || 0;
    if (accidents <= 0) return 0;
    return Number(((fatalities / accidents) * 100).toFixed(2));
  };
  ...
  const fatalityBarCard = category === 'cluster'
    ? {
        title: '% Fatality Accident by State',
        max: Math.max(fatalityPercent(a), fatalityPercent(b), fatalityPercent(c)),
        values: [fatalityPercent(a), fatalityPercent(b), fatalityPercent(c)]
      }
    : {
        title: 'Fatalities',
  ...
  return {
    bars: [
      ...
      {
        title: 'Fatal + Grievous Accidents',
...
const normalizeAlphaKey = (value) =>
  String(value || '')
    .toLowerCase()
    .replace(/[^a-z]/g, '');

const canonicalStateKey = (value) => {
  const key = normalizeAlphaKey(value);
  return SYNONYMS[key] || key;
};

const findMatchingStateKey = (source = {}, stateName = '') => {
  const requestedNormalized = normalizeKey(stateName);
  const requestedCanonical = canonicalStateKey(stateName);

  return Object.keys(source || {}).find((key) => {
    return normalizeKey(key) === requestedNormalized || canonicalStateKey(key) === requestedCanonical;
  }) || null;
};

const findMatchingDistrictKey = (source = {}, districtName = '') => {
  const requestedNormalized = normalizeKey(districtName);
  return Object.keys(source || {}).find((key) => normalizeKey(key) === requestedNormalized) || null;
};

const getClusterTopRowsFromScopedJson = ({ stateName = null, districtName = null } = {}) => {
  if (!stateName) return [];

  if (districtName) {
    const matchedStateKey = findMatchingStateKey(clusterDistrictTopData, stateName);
    if (!matchedStateKey) return [];

    const districtMap = clusterDistrictTopData?.[matchedStateKey] || {};
    const matchedDistrictKey = findMatchingDistrictKey(districtMap, districtName);
    if (!matchedDistrictKey) return [];

    return (districtMap?.[matchedDistrictKey] || []).map((row) => ({
      ...row,
      state: stateName,
  ...
  const matchedStateKey = findMatchingStateKey(clusterStateTopData, stateName);
  if (!matchedStateKey) return [];

  return clusterStateTopData?.[matchedStateKey] || [];
};

const resolveStateDataFromMap = (stateDataMap, stateName) => {
  if (!stateDataMap || !stateName) return null;

  const directMatch = stateDataMap[String(stateName).toLowerCase()];
  if (directMatch) return directMatch;

  const requestedNormalized = normalizeKey(stateName);
  const requestedCanonical = canonicalStateKey(stateName);

  for (const [key, value] of Object.entries(stateDataMap)) {
    if (normalizeKey(key) === requestedNormalized) return value;
    if (canonicalStateKey(key) === requestedCanonical) return value;
  }

  return null;
};

    const getStateIndiaSummary = (stateName) => {
  const stateKey = normalizeKey(stateName);

  const matchedEntry = Object.entries(
    mortStateIndiaSummaryData?.states || {}
  ).find(([key]) => normalizeKey(key) === stateKey);

  return matchedEntry ? matchedEntry[1] : {};
};

const getDistrictIndiaSummary = (stateName, districtName) => {
  const stateKey = normalizeKey(stateName);
  const districtKey = normalizeKey(districtName);

  const matchedState = Object.entries(
    mortDistrictIndiaSummaryData?.states || {}
  ).find(([key]) => normalizeKey(key) === stateKey);

  if (!matchedState) return {};

  const districts = matchedState[1]?.districts || {};

  const matchedDistrict = Object.entries(districts).find(
    ([key]) => normalizeKey(key) === districtKey
  );

  return matchedDistrict ? matchedDistrict[1] : {};
};
...
const cloneBars = (bars = []) =>
  (bars || []).map((bar) => ({
    ...
    rows_expanded: (bar.rows_expanded || []).map((row) => ({ ...row }))
  }));
...
const getStateLevelBarsFromConfig = (category = DEFAULT_CATEGORY, stateName = null) => {
  ...
  const stateKey = normalizeKey(stateName);
  const stateEntries = config?.states
  ? Object.entries(config.states)
  : Object.entries(config || {}).filter(([key]) => key !== 'default');

const matchedStateEntry = stateEntries.find(
  ([key]) => normalizeKey(key) === stateKey
);

const stateBars = matchedStateEntry ? matchedStateEntry[1] : null;
...
const getDistrictLevelBarsFromConfig = (
  ...
  const stateKey = normalizeKey(stateName);
  const districtKey = normalizeKey(districtName);
  const stateEntries = config?.states ? Object.entries(config.states) : [];
  const matchedStateEntry = stateEntries.find(([key]) => normalizeKey(key) === stateKey);
  const stateConfig = matchedStateEntry ? matchedStateEntry[1] : {};

  const districtEntries = Object.entries(stateConfig?.districts || {});
  const matchedDistrictEntry = districtKey
    ? districtEntries.find(([key]) => normalizeKey(key) === districtKey)
    : null;
  const districtBars = matchedDistrictEntry ? matchedDistrictEntry[1] : null;
...
const getManualDistrictData = (districtConfig, stateName, districtName, baseState) => {
  ...
  return {
    ...baseState,
    ...manualDistrict,
    india_summary: getDistrictIndiaSummary(stateName, districtName),
...
const getMatchedDistrictFactor = (districtConfig, stateName, districtName) => {
  const districtKey = normalizeKey(districtName);
  const stateFactors = getDistrictFactorsForState(districtConfig, stateName);
  if (!stateFactors || typeof stateFactors !== 'object') return null;

  const matchedFactor = Object.entries(stateFactors).find(
    ([name]) => normalizeKey(name) === districtKey
  )?.[1];

  return typeof matchedFactor === 'number' && matchedFactor > 0
    ? matchedFactor
    : null;
};
...
export const getDistrictData = (stateName, districtName, category = DEFAULT_CATEGORY) => {
  ...
  const baseState = resolveStateDataFromMap(stateDataMap, stateName);
  ...
  const districtIndiaSummary = getDistrictIndiaSummary(stateName, districtName);
...
export const getDistrictComparisonRows = (stateName, category = DEFAULT_CATEGORY, districtNames = []) => {
  ...
  */
  ...
  const namePool = [...manualNames, ...configuredNames];
...
export const getDataByLevel = ({
  ...
  // District data is always scoped by state. If state is missing, treat as no data
  // instead of falling back to national summary.
  ...
  if (stateName) {
  const stateData = resolveStateDataFromMap(getStateDataMap(category), stateName) || null;

  if (!stateData) return null;

    return {
      ...stateData,
      india_summary: getStateIndiaSummary(stateName)
    };
  }
...
const normalizeSchoolZoneDistrictBars = (bars = [], selectionData = null) => {
  if (!Array.isArray(bars) || !selectionData) return bars;

  const ageMetric = selectionData?.school_zone_metrics?.accidents_age_6_18;
  const ageRows = Array.isArray(ageMetric)
    ? ageMetric.map((row) => ({
        label: row?.label || '6-18 yrs',
        value: Number(row?.value) || 0
      }))
    : Number.isFinite(Number(ageMetric))
      ? [{ label: '6-18 yrs', value: Number(ageMetric) || 0 }]
      : [];

  if (!ageRows.length) return bars;
  ...
  return bars.map((bar) => {
    ...
    if (!isAgeGroupBar) return bar;

    return {
      ...bar,
      rows: ageRows,
      rows_expanded: ageRows
    };
  });
};
...
const buildSelectionDashboardData = ({
  ...
  if (stateName) {
    ...
    if (districtRows && districtRows.length > 0) {
      if (districtName) {
        ...
        // state selected (no district): show districts for that state
        scopedStates = [...districtRows]
          .sort((a, b) => (b.fg_covered || 0) - (a.fg_covered || 0));

        if (category !== 'cluster') {
          scopedStates = scopedStates.slice(0, 8);
        }
      }
    }
  }
  ...
  const normalizedScopedBars =
    category === 'schools-zone' && districtName
      ? normalizeSchoolZoneDistrictBars(scopedBars, selectionData)
      : scopedBars;

  const clusterScopedTopRows = category === 'cluster'
    ? getClusterTopRowsFromScopedJson({ stateName, districtName })
    : [];

  return {
    ...baseDashboard,
    india_summary: baseDashboard.india_summary,
    summary: {
    ...baseDashboard.summary,
    total_length_processed: toIndianString(selectionData.total_length_processed || 0, 1),
    fatal_grievous_covered: toIndianString(selectionData.fg_covered || 0),
    total_fatal_accidents: toIndianString(selectionData.total_fatal_accidents || 0),
    total_grievous_accidents: toIndianString(selectionData.total_grievous_accidents || 0),
    blackspots: toIndianString(selectionData.blackspots || 0),
    fg_coverage: toIndianString(selectionData.total_fg || 0)
  },
    states: scopedStates,
    agencies:  Array.isArray(selectionData.agencies) && selectionData.agencies.length
    ? selectionData.agencies.map((agency) => ({
        ...agency,
        length: toIndianString(agency.length || 0, 1),
        corridors: toIndianString(agency.corridors || 0),
        accidents: toIndianString(agency.accidents || 0),
        fatalities: toIndianString(agency.fatalities || 0)
      }))
    : [
        {
          name: scopeName,
          length: toIndianString(selectionData.total_length_processed || 0, 1),
    ...
    bars: normalizedScopedBars,
    top20Corridors:
  districtName
    ? (
        selectionData?.top_corridors ||
        selectionData?.top20Corridors ||
        []
      )
    : stateName
      ? (
          selectionData?.top_corridors ||
          selectionData?.top_20_corridors ||
          selectionData?.top20Corridors ||
          baseDashboard.top20Corridors ||
          []
        )
      : (baseDashboard.top20Corridors || [])
    ,
  ...
  districtName
    ? (
        (clusterScopedTopRows.length ? clusterScopedTopRows : null) ||
        selectionData?.top_clusters ||
        selectionData?.top20Clusters ||
        baseDashboard.top20Clusters ||
        []
      )
    : stateName
      ? (
...
// ═══════════════════════════════════════════════════════════════════════════════
// Cluster Violations Data Extraction Functions
// ═══════════════════════════════════════════════════════════════════════════════

const getClusterViolationsTopNational = (limit = 5) => {
  const violationMap = {};
  
  if (!clusterDistrictViolationsData || typeof clusterDistrictViolationsData !== 'object') {
    return [];
  }

  // Iterate through all states
  Object.entries(clusterDistrictViolationsData).forEach(([stateName, districts]) => {
    if (districts && typeof districts === 'object') {
      // Iterate through all districts in the state
      Object.entries(districts).forEach(([districtName, violations]) => {
        if (Array.isArray(violations)) {
          // Aggregate violations across all districts
          violations.forEach((violation) => {
            const name = violation?.name || '';
            if (name && name !== 'No Violation') {
              const count = Number(violation?.count) || 0;
              violationMap[name] = (violationMap[name] || 0) + count;
            }
          });
        }
      });
  ...
  return Object.entries(violationMap)
    .map(([name, count]) => ({ name, value: count }))
    .sort((a, b) => b.value - a.value)
    .slice(0, limit);
};

const getClusterViolationsTopByState = (stateName, limit = 5) => {
  if (!stateName || !clusterDistrictViolationsData) return [];

  const matchedStateKey = findMatchingStateKey(clusterDistrictViolationsData, stateName);
  if (!matchedStateKey) return [];

  const violationMap = {};
  const districts = clusterDistrictViolationsData[matchedStateKey] || {};

  // Aggregate violations across all districts in the state
  Object.entries(districts).forEach(([districtName, violations]) => {
    if (Array.isArray(violations)) {
      violations.forEach((violation) => {
        const name = violation?.name || '';
...
const getClusterViolationsTopByDistrict = (stateName, districtName, limit = 5) => {
  if (!stateName || !districtName || !clusterDistrictViolationsData) return [];

  const matchedStateKey = findMatchingStateKey(clusterDistrictViolationsData, stateName);
  if (!matchedStateKey) return [];

  const districtMap = clusterDistrictViolationsData[matchedStateKey] || {};
  const matchedDistrictKey = findMatchingDistrictKey(districtMap, districtName);
  if (!matchedDistrictKey) return [];

  const violations = districtMap[matchedDistrictKey] || [];
  
  // Filter out "No Violation" and sort by count descending
  return violations
    .filter((violation) => violation?.name !== 'No Violation')
    .map((violation) => ({
      name: violation?.name || '',
      value: Number(violation?.count) || 0
    }))
    .sort((a, b) => b.value - a.value)
    .slice(0, limit);
};

const getClusterFgCountByState = (stateName) => {
  if (!stateName || !clusterDistrictFgData) return 0;

  const matchedStateKey = findMatchingStateKey(clusterDistrictFgData, stateName);
  if (!matchedStateKey) return 0;

  const districtCounts = clusterDistrictFgData[matchedStateKey] || {};
  
  // Sum all FG counts for the state
  return Object.values(districtCounts).reduce((sum, count) => sum + Number(count || 0), 0);
};

const getClusterFgCountByDistrict = (stateName, districtName) => {
  if (!stateName || !districtName || !clusterDistrictFgData) return 0;

  const matchedStateKey = findMatchingStateKey(clusterDistrictFgData, stateName);
  if (!matchedStateKey) return 0;

  const districtMap = clusterDistrictFgData[matchedStateKey] || {};
  const matchedDistrictKey = findMatchingDistrictKey(districtMap, districtName);
  if (!matchedDistrictKey) return 0;

  return Number(districtMap[matchedDistrictKey]) || 0;
};
...
const buildSchoolZoneComparisonBars = (seriesRows) => {
  ...
  */
  ...
  const age618Values = seriesRows.map((row) => {
    const metrics = row.state?.school_zone_metrics || {};
    const aggregated = sumMetricValues(metrics.accidents_age_6_18);
    return aggregated || Number(row.state?.total_fatalities) || 0;
  });
  */}

  const age618Values = seriesRows.map((row) => {
    const metrics = row.state?.school_zone_metrics || {};
    return sumMetricValues(metrics.accidents_age_6_18);
  });
  ...
  return [
    buildBar('Distribution of accidents mapped to schools', perBufferValues),
    buildBar('Accidents by Age Groups', age618Values),
...
const buildChartEntry = (label, seriesRows, category = DEFAULT_CATEGORY) => {
  ...
  const cardData = buildComparisonCardData(seriesRows, category);
  return {
    label,
    series: seriesRows.map((row) => ({ label: row.label, color: row.color })),
    line: null,
...
// Export cluster violations and FG data functions
export const getClusterTopViolations = {
  national: () => getClusterViolationsTopNational(5),
  byState: (stateName) => getClusterViolationsTopByState(stateName, 5),
  byDistrict: (stateName, districtName) => getClusterViolationsTopByDistrict(stateName, districtName, 5)
};

export const getClusterFgCount = {
  byState: (stateName) => getClusterFgCountByState(stateName),
  byDistrict: (stateName, districtName) => getClusterFgCountByDistrict(stateName, districtName)
};
