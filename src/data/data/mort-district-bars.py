...
ORDER BY state_key, district_key, sort_order;
"""
# ---------------------------------------------------------
...
ORDER BY state_key, district_key, value DESC;
"""
...
FROM (
    SELECT
        ...
        LOWER(TRIM(districts)) AS district_key,
        '2023' AS label,
        COALESCE(SUM(fatal_grevious_accidents_count_2023), 0) AS value
    FROM corridor_30m
    WHERE states IS NOT NULL
      AND TRIM(states) <> ''
      AND districts IS NOT NULL
      AND TRIM(districts) <> ''
    GROUP BY LOWER(TRIM(states)), LOWER(TRIM(districts))
...
# ---------------------------------------------------------
# Get all state/district combinations
# ---------------------------------------------------------
cursor.execute("""
SELECT DISTINCT
    LOWER(TRIM(state)) AS state_key,
...
WHERE morth_corridor_id_30m IS NOT NULL
  ...
  AND state IS NOT NULL
  AND district IS NOT NULL
ORDER BY state_key, district_key
""")
    ...
    result["states"][state_key]["districts"][district_key] = [
        {
            ...
            "rows": crash_corridor_lookup.get((state_key, district_key), [])
        },

        {
            "title": "Crashes by Year",
            "rows": year_lookup.get((state_key, district_key), [])
        },
        {
            "title": "Fatalities per Corridor",
            "rows": fatality_corridor_lookup.get((state_key, district_key), [])
        },
        {
            "title": "Crashes by Time",
            "rows": time_lookup.get((state_key, district_key), [])
        },
        {
            "title": "Crashes by Junction Type",
            "rows": sorted(
                junction_lookup.get((state_key, district_key), []),
        ...
        },
        {
            ...
            "rows": sorted(
                vehicle_lookup.get((state_key, district_key), []),
                key=lambda x: x["value"],
                reverse=True
            )
        },
        {
            ...
            "rows": sorted(
                pedestrian_accident_lookup.get((state_key, district_key), []),
                key=lambda x: x["value"],
                reverse=True
            )
        },
        {
            ...
            "rows": sorted(
                pedestrian_fatality_lookup.get((state_key, district_key), []),
                key=lambda x: x["value"],
                reverse=True
            )
        }
...
# ---------------------------------------------------------
# Default section from all districts combined
# ---------------------------------------------------------
...
with open("mort-district-bar-1.json", "w", encoding="utf-8") as f:
    json.dump(result, f, indent=2, ensure_ascii=False)

print("mort-district-bar-1.json generated successfully.")
