...
FROM state_accidents da
LEFT JOIN state_corridors dc
    ON da.state_key = dc.state_key
ORDER BY da.state_name;
"""

print("Loading state summary...")
...
# ---------------------------------------------------------
# ---------------------------------------------------------
# Preload top violations for all states
# ---------------------------------------------------------
violations_query = """
WITH expanded AS (
    SELECT
...
ranked AS (
    SELECT *,
        ROUND(
            count * 100.0 /
            SUM(count) OVER (PARTITION BY state_key),
            1
        ) AS pct
    FROM grouped
)
...
# ---------------------------------------------------------
# ---------------------------------------------------------
# Preload top crash types for all states
# ---------------------------------------------------------
crash_type_query = """
WITH expanded AS (
    SELECT
...
# ---------------------------------------------------------
# ---------------------------------------------------------
# Preload top crash natures for all states
# ---------------------------------------------------------
crash_nature_query = """
WITH expanded AS (
    SELECT
...
# ---------------------------------------------------------
for row in tqdm(state_rows, desc="Processing States"):
...
# Save JSON file
# ---------------------------------------------------------
with open("mort-states-1.json", "w", encoding="utf-8") as f:
    json.dump(result, f, indent=2, ensure_ascii=False)

print("mort-states-1.json generated successfully.")
